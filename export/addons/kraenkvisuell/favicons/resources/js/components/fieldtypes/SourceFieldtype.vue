...
const sourceTypeSelectOptions = computed(() => {
    ...
    if (props.config.field !== false) {
        options.push({ label: __('favicons::messages.custom'), value: 'custom' })
    }

    if (props.config.from_field !== false) {
        options.unshift({ label: __('favicons::messages.from_field'), value: 'field' })
    }

    if (props.config.inherit !== false) {
        options.unshift({ label: __('favicons::messages.inherit'), value: 'inherit' })
    }

    if (props.config.disableable) {
        options.push({ label: __('favicons::messages.disable'), value: 'disable' })
    }
...
const sourceFieldOptions = computed(() => {
    ...
    const allowedFieldtypes = ['text', 'textarea', 'markdown', 'bard', ...(props.config.allowed_fieldtypes ?? [])]
...
<template>
    <div
        class="flex flex-col md:flex-row md:items-center @max-lg/live-preview:!flex-col @max-lg/live-preview:!items-start gap-2 border border-gray-200 rounded-md p-2 bg-gray-50 dark:bg-gray-900 dark:border-gray-800"
    >
