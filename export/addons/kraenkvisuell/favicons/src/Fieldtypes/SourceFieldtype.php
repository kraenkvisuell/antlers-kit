...
use Kraenkvisuell\Favicons\Fieldtypes\Rules\SourceFieldRule;
use Statamic\Support\Str;

class SourceFieldtype extends Fieldtype
{
    public static $handle = 'favicons_source';

    public $selectable = false;

    public function preProcess($data)
    {
        if (is_string($data) && Str::startsWith($data, '@favicons:')) {
            return ['source' => 'field', 'value' => explode('@favicons:', $data)[1]];
        }
    ...
    public function process($data)
    {
        ...
        if ($data['source'] === 'field') {
            return $data['value'] ? '@favicons:' . $data['value'] : null;
        }
