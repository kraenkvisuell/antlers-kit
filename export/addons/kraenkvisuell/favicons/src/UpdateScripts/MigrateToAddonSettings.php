...
namespace Kraenkvisuell\Favicons\UpdateScripts;
...
class MigrateToAddonSettings extends UpdateScript
{
    ...
    {
        if (! File::exists($path = config('statamic.favicons.site_defaults.path', base_path('content/favicons.yaml')))) {
            return $this;
        }

        Addon::get('kraenkvisuell/favicons')->settings()->set($this->getSettingsData($path))->save();
